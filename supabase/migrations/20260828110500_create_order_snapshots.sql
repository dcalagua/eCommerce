-- =============================================================================
-- P08-SaaS · 6/7 — El alta del pedido escribe el SNAPSHOT completo
--
-- Esta migracion no cambia ni una regla de negocio del checkout: mismo precio,
-- mismo impuesto, mismo canal, misma existencia, mismos errores y mismos
-- codigos. Lo unico que cambia es **cuanto se congela** en el momento de la
-- compra, que es exactamente lo que pregunta el criterio de aceptacion de la
-- fase: «el historial de un pedido sigue siendo correcto aunque cambien
-- producto, precio, impuestos o configuracion despues de comprar».
--
-- Lo que antes no se guardaba y ahora si:
--
--  | Dato                    | Antes                    | Ahora                       |
--  |-------------------------|--------------------------|-----------------------------|
--  | tasa e importe de IVA   | solo el total del pedido | tambien por LINEA           |
--  | categoria fiscal        | uuid vivo en `products`  | codigo congelado en la linea|
--  | variante                | dentro del texto `name`  | etiqueta + atributos        |
--  | receta del kit          | nada                     | componentes congelados      |
--  | codigo de lista         | uuid que se anula        | codigo que sobrevive        |
--  | impuesto incluido       | `store_settings` (vivo)  | columna del pedido          |
--  | direccion del comprador | editable                 | + copia inmutable           |
--  | direccion fiscal        | no existia               | congelada                   |
--  | cliente                 | correo suelto            | snapshot con documento      |
--  | origen                  | no existia               | `source_channel`            |
--  | aprobacion B2B          | no existia               | `approval_status`           |
--
-- ## Por que se DEJA CAER y se recrea en vez de `create or replace`
--
-- Porque la firma crece con cuatro parametros. `create or replace` con otra
-- firma no reemplaza: **crea una sobrecarga**, y entonces cada llamada de ocho
-- argumentos se vuelve ambigua o —peor— resuelve a la version vieja y el
-- snapshot no se escribe nunca sin que nada falle. Es el mismo procedimiento
-- que uso P06 al añadir `p_reservation_token`, y por la misma razon.
--
-- Todos los parametros nuevos llevan `default`, asi que las llamadas de ocho
-- argumentos que ya existen —`create_order_for_slug`, `checkout_place_order` y
-- todos los tests de P02 a P07— siguen compilando y siguen dando el mismo
-- resultado. Un pedido sin cuenta corporativa nace `not_required` y por ahi no
-- pasa ni una linea de codigo B2B: el circuito de aprobacion no contamina B2C.
-- =============================================================================

drop function if exists public.create_order(uuid, text, jsonb, text, text, jsonb, text, text);
...
begin
  ...
  --
  -- `v_tax` se redondea por GRUPO DE TASA (justo arriba, sin cambios desde
  -- P02). Guardar en cada linea `round(importe x tasa, 2)` daria una suma que
  -- puede diferir del total del pedido en unos centimos: la factura no cuadraria
  -- consigo misma y nadie sabria cual de los dos numeros es el bueno.
  --
  -- Lo que se hace es repartir el total del GRUPO entre sus lineas en
  -- proporcion al importe y dar el residuo del redondeo a la linea mas grande
  -- del grupo (metodo del resto mayor). La suma de las lineas es EXACTAMENTE el
  -- `tax_total` del pedido por construccion, no por suerte, y hay un test que
  -- lo comprueba con tres lineas de importes que no se dividen bien.
  with lineas as (
    select ord,
           line,
           (line ->> 'tax_rate')::numeric as rate,
           (line ->> 'amount')::numeric   as gross
  ...
  grupos as (
    select rate,
           ...
           case when v_inclusive
                then round(sum(gross) - sum(gross) / (1 + rate), 2)
                else round(sum(gross) * rate, 2)
           end as tax
    from lineas
    group by rate
  ),
  repartido as (
    select l.ord,
           ...
           l.rate,
           g.tax as group_tax,
           case when g.gross = 0 then 0
                else round(g.tax * l.gross / g.gross, 2)
           end as line_tax,
           row_number() over (partition by l.rate order by l.gross desc, l.ord) as rn
    from lineas l
...
revoke execute on function
  public.create_order(uuid, text, jsonb, text, text, jsonb, text, text, text, uuid, jsonb, jsonb)
...
comment on function
  public.create_order(uuid, text, jsonb, text, text, jsonb, text, text, text, uuid, jsonb, jsonb) is
  'Crea el pedido y congela su SNAPSHOT completo (impuesto por linea, variante, receta de kit, direcciones y cliente). Ni precio, ni canal, ni lista, ni factor, ni almacen, ni tenant, ni origen se aceptan del navegador.';

-- ---------------------------------------------------------------------------
-- create_order_for_slug — misma razon para dejarla caer, y sigue sin duplicar
-- ni una linea de logica. Los parametros nuevos se ARRASTRAN: el comprador
-- anonimo de la vitrina no declara ninguno, pero el pipeline de checkout si
-- pasa por aqui con la cuenta B2B que resolvio con la sesion.
-- ---------------------------------------------------------------------------
drop function if exists public.create_order_for_slug(text, text, jsonb, text, text, jsonb, text, text);
...
revoke execute on function
  public.create_order_for_slug(text, text, jsonb, text, text, jsonb, text, text, text, uuid, jsonb, jsonb)
from public, anon, authenticated;
...
comment on function
  public.create_order_for_slug(text, text, jsonb, text, text, jsonb, text, text, text, uuid, jsonb, jsonb) is
  'Checkout publico: resuelve la tienda por slug (solo activa) y delega en create_order, arrastrando reserva, origen, cuenta B2B y direccion fiscal. Solo service_role.';

-- ===========================================================================
-- checkout_place_order — la transaccion que cierra el checkout, con el
-- contexto B2B que solo el borde podia averiguar
--
-- Sigue haciendo lo mismo y en el mismo orden: pedido + intento + carrito +
-- hechos, o ninguna de las cuatro. Lo que se añade son tres datos que el
-- orquestador resolvio ANTES, fuera de esta transaccion y con la sesion del
-- comprador delante:
--
--  · `p_business_account_id` — de que cuenta corporativa compra. Sale de
--    `my_business_accounts()`, llamada con el JWT del comprador, porque esa
--    funcion no acepta un id de cuenta desde P05. Con `service_role` la
--    pregunta no tendria respuesta posible: no hay sesion que consultar.
--  · `p_approval` — lo que dijo `public.purchase_approval` sobre el LIMITE DE
--    LA PERSONA, que tampoco se puede saber sin sesion. El umbral de la CUENTA
--    lo sigue decidiendo `create_order` con la fila delante.
--  · `p_billing_address` — la direccion fiscal, si la hubo.
--
-- Y un hecho nuevo en el outbox: cuando el pedido nace esperando autorizacion,
-- se publica `order.approval_requested`. Sin eso, el aprobador se entera
-- cuando alguien le escriba.
-- ===========================================================================

drop function if exists public.checkout_place_order(uuid, text, jsonb, text, text, jsonb, text, text, jsonb);
...
revoke execute on function
  public.checkout_place_order(uuid, text, jsonb, text, text, jsonb, text, text, jsonb, uuid, jsonb, jsonb)
from public, anon, authenticated;
...
comment on function
  public.checkout_place_order(uuid, text, jsonb, text, text, jsonb, text, text, jsonb, uuid, jsonb, jsonb) is
  'La transaccion que cierra el checkout: pedido + intento + carrito + hechos, o ninguna de las cuatro. Sin una sola llamada externa dentro. Desde P08 arrastra cuenta B2B, direccion fiscal y aprobacion.';

-- ===========================================================================
-- order_by_token — el comprador ve sus CUATRO estados, y nada mas
--
-- Se amplia la proyeccion con `payment_status` y `fulfillment_status`, que es
-- justo lo que el comprador pregunta por telefono («¿ya se cobro?», «¿ya
-- salio?»), y con `approval_status`, para que un comprador B2B sepa que su
-- pedido espera la firma de alguien de su empresa.
--
-- Lo que sigue SIN salir, y es la parte importante: notas internas, etiquetas,
-- referencias externas, ids de tenant, el id interno del pedido y el token. La
-- funcion no lee ninguna de las tablas de la migracion 110300 y no debe leerlas
-- nunca: «cliente moroso» y «revisar por fraude» son anotaciones del comercio.
-- ===========================================================================
...
comment on function public.order_by_token(text, text, text) is
  'Unica puerta del comprador anonimo a su pedido. Exige tienda activa + numero + token; no distingue "no existe" de "token incorrecto". Desde P08 devuelve los cuatro ejes de estado y sigue sin devolver notas internas, etiquetas ni referencias externas.';
